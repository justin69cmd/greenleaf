...
export interface ProgressiveFluxLoaderProps {
  /** Controlled progress, `0`–`100`. Omit to let the loader run its own sweep. */
  value?: number
  /** Phase thresholds and their labels. */
  ...
  showLabel?: boolean
  /** CSS background for the bar fill. Defaults to the purple flux gradient. */
  gradient?: string
...
// Signature flux palette — recolored to Equilibrium's gold. Override per
// instance via the `--flux-from` / `--flux-to` CSS variables or the `gradient` prop.
const FLUX_FROM = 'var(--flux-from, #f59e0b)'
const FLUX_TO = 'var(--flux-to, #fde68a)'
...
const BAR_SHADOW = `0 0 18px color-mix(in oklab, ${FLUX_FROM} 55%, transparent), 0 0 32px color-mix(in oklab, ${FLUX_TO} 40%, transparent), inset 0 1.5px 0 rgba(255, 255, 255, 0.5), inset 0 -2px 3px rgba(60, 30, 0, 0.45)`
...
function FluxLabel({ label, reduced, className }: FluxLabelProps) {
  const base = cn(
    'absolute inset-0 flex items-center justify-center text-center text-3xl font-semibold tracking-tight text-amber-100/80 sm:text-4xl',
    className
  )
...
export function ProgressiveFluxLoader({
  ...
  return (
    <div className={cn('mx-auto flex w-full max-w-md flex-col items-center gap-8', className)}>
      ...
      <div
        className={cn(
          'relative h-5 w-full overflow-hidden rounded-full bg-white/10 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]',
          barClassName
        )}
        role="progressbar"
        aria-valuemin={0}
